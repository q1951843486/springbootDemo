package com.cn.demo.arangoSpark.commons;
...
/**
 * @Description
 * @ClassName SparkOperationArangoDbUtils
...
public class SparkOperationArangoDbUtils {
    ...
    public static <T> void  loadArangoDbToSpark(String DB,String LoadCollection,Class<T> loadArangoDBClass){
        ...
        ArangoJavaRDD<T> rdd = ArangoSpark.load(sc, LoadCollection, new ReadOptions().database(DB).protocol(Protocol.HTTP_JSON),
                loadArangoDBClass);
    ...
    public  static <T> List RowToJavaBean(Dataset<Row> dataset,Class<T> querycollectionClass){
        ...
        for (Row r : rows) {
            ...
            list.add(jsonObject.toJavaObject((Type) querycollectionClass));
